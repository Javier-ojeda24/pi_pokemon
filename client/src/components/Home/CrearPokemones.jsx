...
import "./css/crearPokemones.css";

export const CrearPokemones = () => {
	...
	function handleSubmit(e) {
		...
		if (!input.name) {
			...
			exist.find(p => p.name.toLowerCase() === input.name.toLowerCase())
		) {
			return alert(
				`*El nombre ingresado -${input.name}- ya existe,ingrese otro`
			);
		} else if (!input.hp) {
	...
	function validate(input) {
		...
		if (!input.name) {
			...
			errors.hp =
				"Los valores de vida no pueden ser negativos o mayor a 200 hp";
		} else if (!input.attack) {
			errors.attack =
				"Es necesario ingresar un valor para el ataque del pokemon";
		} else if (input.attack < 0 || input.attack > 150) {
			errors.attack =
				"Los valores de ataque no pueden ser negativos o mayor a 150 de ataque";
		} else if (!input.defense) {
			errors.defense = "Es necesario un valor para la defensa del pokemon";
		} else if (input.defense < 0 || input.defense > 150) {
			errors.defense =
				"Los valores de ataque no pueden ser negativos o mayor a 150 de defensa";
		} else if (!input.speed) {
			errors.speed = "Es necesario un valor para la velocidad del pokemon";
		} else if (input.speed < 0 || input.speed > 110) {
			errors.speed =
				"Los valores de ataque no pueden ser negativos o mayor a 110 de velocidad";
		} else if (!input.height) {
			errors.height = "*Ingrese un valor para la altura del pokemon";
		} else if (input.height < 0 || input.height > 100) {
			errors.height =
				"Los valores de ataque no pueden ser negativos o mayor a 100 para la altura del pokemon";
		} else if (!input.weight) {
			errors.weight = "Ingrese un valor para el peso del pokemon";
		} else if (input.weight < 0 || input.weight > 1000) {
			errors.weight =
				"Los valores de ataque no pueden ser negativos o mayor a 1000 para el peso del pokemon";
		} else if (!input.image) {
			errors.image =
				"Ingrese una imagen, de lo contrario sera creado el pokemon con la imagen por defecto";
		}
		...
		<div className="app-crear">
			<div className="container_all">
				...
				<form onSubmit={e => handleSubmit(e)}>
					...
					<button type="submit" className="btn_home">
						Crear Pokemon
					</button>
				</form>
			</div>
